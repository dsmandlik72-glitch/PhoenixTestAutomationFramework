	...
	public static JobHeadModel getDataFromJobHead(int tr_customer_id) {
		...
		try {
			...
			while(rs.next()) {
				...
				jobHeadModel=new JobHeadModel(rs.getInt("id"), rs.getString("job_number"), rs.getInt("tr_customer_id"), rs.getInt("tr_customer_product_id"), rs.getInt("mst_service_location_id"), 
